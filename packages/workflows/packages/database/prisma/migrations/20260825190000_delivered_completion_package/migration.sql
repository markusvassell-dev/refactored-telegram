...
-- deliberately not reversible by a stale source document: the documents are in
-- the client's records and this application cannot unsend them. What a stale
-- source does is send the engagement back to COVER_LETTER_CHANGES_REQUESTED so
-- it stops reading as finished until somebody regenerates and approves again.
...
-- an upload that reported success against a mock adapter has filed nothing,
-- and a package with no file ids is how a later reader can tell.
-- AlterTable
