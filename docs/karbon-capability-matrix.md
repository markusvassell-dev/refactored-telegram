...
| List the firm's clients | **Supported** | `GET /v3/Organizations` and `GET /v3/Contacts`, paged with `$top`/`$skip` | — | Observed 2026-08-17: 25 organisations and 25 contacts with real keys, contact types `Client` and `Inactive`. **The same run found the limit was being applied to each endpoint rather than as a total**, so asking for 25 returned 50, and nothing in the result distinguished a truncated list from a complete one — a 50 that was a slice read as a firm's whole book. The limit is now a total split evenly between the two endpoints, and the result carries whether a list was cut short. Splitting rather than taking organisations first is deliberate: a firm with more organisations than the limit would otherwise never see one individual client, and a T1 filer is a `Contact`. **Neither endpoint was ever called until 2026-08-14.** Client discovery ran entirely through work items — search them, take the distinct client keys — which answers "who has work open", not "who are the clients". A new client, a dormant one, and any client whose work predates the window examined were all invisible, and the screen reported the count as clients found in Karbon. Both entity types are read, because a corporation is an Organization and an individual filing a T1 is a Contact. `ContactType` is returned verbatim and the **client never filters on it**: the vocabulary is tenant-defined via `/v3/TenantSettings`, so filtering for `'Client'` would silently drop every client of a firm that calls them something else. The import excludes `Inactive` and `Dissolved` — **the firm's decision about its own labels, taken after the 2026-08-17 run reported which types exist, not an inference.** A dissolved corporation has no legal existence and cannot be a party to an engagement letter; `Other` is kept, because it says nothing about whether the entry is a client. The import reports how many it skipped and under which label, and can be told to include every type for a current client left mislabelled in Karbon. A tenant using different words needs `EXCLUDED_CONTACT_TYPES` changed; a value there that a tenant does not use excludes nothing rather than failing. Work-item discovery remains available and now says what it is. |
| Search Karbon's client list by name | **Not used** | would be `GET /v3/Organizations?$filter=…` | The client list is read whole, and Karbon's unsplit `FullName` is stored on each client so the lookup runs against this application's own database | **The one query a client lookup wants is the one Karbon does not document.** Verified against the published spec 2026-08-17: `/v3/Contacts` documents `$filter` with `eq`, `contains` and `and` over `FullName`, `EmailAddress` and `PhoneNumber` — but `/v3/Organizations` documents **only `eq` and `and`**, over `FullName`, `EmailAddress` and `ExternalKey`. `contains` appears in the Organizations examples for `ContactType` and **never for `FullName`**. The firm's clients are largely numbered corporations, which are Organizations, so partial-name search against Karbon is undocumented for exactly the entities that need it. An undocumented `$filter` is not safely a 400: it may be **ignored**, returning the first hundred organisations as though they matched — the silent-success failure this matrix exists to prevent. It is also unnecessary: `GET /v3/Organizations` and `GET /v3/Contacts` already return `FullName` for every entity with no per-client read, so the import keeps that string on the client record (`client.karbonFullName`) and the lookup is a local query. Freshness comes from re-running the import, which is idempotent. |
